...
async function addCreatedAtToExistingDocuments() {
  try {
    const products = await Tool.find().sort({ _id: 1 }); // Sort by _id to ensure ordering
    ...
    for (const product of products) {
      ...
      await Tool.findByIdAndUpdate(product._id, { createdAt: timestamp });
      console.log(
...
async function updateProducts() {
  try {
    ...
    const updatedResult = await Tool.updateMany(
